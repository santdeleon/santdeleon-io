...
const Footer = ({socialIcons, copyright, heart}) => {
  return (
      ...
      <div className="footer-wrapper flex align-items-center justify-content-between">
        <div className="footer-social-icons flex align-items-center">
          {Object.keys(socialIcons).map(icon => {
            return  <a href={socialIcons[icon].url} key={socialIcons[icon].id} className="social-icon-container flex" target="_blank" rel="noopener noreferrer" aria-label={icon} title={`${icon} link`}>
        ...
        <div className="flex align-items-center heart-wrapper">
          <h4 className="m-0 flex align-items-center fw-normal">Made with <FontAwesomeIcon className="social-icon heart" icon={heart} /> by Sant</h4>
        </div>
      </div>
    </footer>
  );
